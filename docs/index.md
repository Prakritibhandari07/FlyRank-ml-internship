...
Use the model's ranked queue for larger review batches (K ≥ 100), not the smallest ones. That's where its precision edge over the hand-written rule (0.57–0.58 vs. 0.51–0.54) is real and worth the added complexity; at K=20–50 the simpler rule performs about as well.
Route page_3_5-tier flags to a ranking review, not a CTR/snippet review. Both methods over-flag this tier for the wrong reason — CTR is structurally capped at low rank positions, so the real lever there is improving rank, not rewriting a snippet.
Never report the random-split number. Precision@20 of 0.90 is real code, correctly run, on a genuinely wrong validation design — always pair any reported precision@K with the split methodology and the test-set base rate.
Reproducibility
